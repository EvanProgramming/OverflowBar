...
The scanner deliberately uses public Accessibility APIs and only includes controls exposed by third-party apps on the right side of the menu bar. Apps that do not expose their status item cannot be mirrored. The implementation does not alter the system menu bar, use private APIs, or attempt real-time capture.
